...
There,
we've seen that we can use predefined SDK to set up
...
While we used the ROS 2 demos in part 1,
...
and create an hidden `.workshop` sub-folder at the project root directory.
...
## Initiating a project SDK

In order to tweak the development environment,
we are making use of 'project SDK'.
A project SDK is an SDK that is distributed alongside a project rather than being
available from the store.

We proceed to create the project SDK,
...
At this point we have a valid project project SDK, readily usable,
albeit empty.
...
Project SDKs are not packaged like other SDKs and
therefore do not make use of `parts`.
They entirely relies on `hooks`.
...
set up the overall environment while the second is used for
...
This may look complex at first glance so let us break this down.
This script:

- make sure that the vcstool file to import exists and exit if it doesn't
  or proceed otherwise
- source the ROS 2 environment
- update rosdep for good measure
- create the underlay workspace and import the sources
- install the underlay workspace dependencies using Rosdep
- build the underlay workspace using Colcon
- source the underlay workspace in the user `.profile` file
...
This prefix tells workshop that it is a project SDK
which definition is local to the project.

From there, we can launch a new workshop or refresh an existing one,
...
Actions are a convenient way to automatize mundane tasks and
serve as a way to exemplify how a project is expected to be used.
...
we can define actions that are entire script.
To do so, we use the pipe symbole in the action definition,
...
However they are not present in the workspace.

If we try to fetch new content from the remote,
we hit an error,
...
namely the `ssh-agent` interface.

To enable it, we simply add the interface to the project SDK definition,
