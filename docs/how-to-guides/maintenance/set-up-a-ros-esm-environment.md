...
Note that ROS ESM is its own ROS distribution, and thus provides its own distribution and `rosdep` files. 
...
Typically, when using ROS ESM, your ROS workspace would already be configured with the relevant source code. 
...
about anything that should be added to ROS ESM. While such additions will need to pass some scrutiny, 
